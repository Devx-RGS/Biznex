...
export default function MemberProfileScreen({ route, navigation }) {
  ...
  const sendEnquiry = () => {
    if (!enquiryText.trim()) {
      Alert.alert('Empty Message', 'Please write your enquiry before sending.');
      return;
    }
    EnquiryStore.sendEnquiry(member, enquiryText);
    setEnquiryText('');
    setEnquiryModal(false);
    Alert.alert('Enquiry Sent', `Your enquiry has been sent to ${member.name}.`);
  };
  ...
  return (
    <View style={s.container}>
      ...
      {/* ── Enquiry Compose Modal ─────────────────────────────────── */}
      <Modal visible={enquiryModal} transparent animationType="slide" onRequestClose={() => setEnquiryModal(false)}>
        <TouchableOpacity style={m.backdrop} activeOpacity={1} onPress={() => setEnquiryModal(false)} />
        <View style={m.sheet}>
          <View style={m.handle} />
          <View style={m.sheetHeader}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={m.toLabel}>Enquiry to</Text>
              <Text style={m.toName}>{member.name}</Text>
              <Text style={m.toBiz}>{member.business}</Text>
            </View>
            <TouchableOpacity onPress={() => setEnquiryModal(false)}>
          ...
          <TextInput
            ...
            autoFocus
          />
          <Text style={m.charCount}>{enquiryText.length}/500</Text>
          <TouchableOpacity
            style={[m.sendBtn, !enquiryText.trim() && m.sendBtnDisabled]}
            onPress={sendEnquiry}
          >
            <Ionicons name="send" size={16} color={enquiryText.trim() ? colors.primary : '#8A9BB0'} />
            <Text style={[m.sendBtnTxt, !enquiryText.trim() && { color: '#8A9BB0' }]}>Send Enquiry</Text>
          </TouchableOpacity>
        </View>
      </Modal>
      ...
      <SafeAreaView style={s.safe} edges={['top']}>
        ...
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={s.scrollContent}>
          ...
          <View style={s.actionRow}>
            ...
            <TouchableOpacity style={s.secondaryActionBtn} onPress={() => setEnquiryModal(true)}>
